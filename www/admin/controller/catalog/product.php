...
class ControllerCatalogProduct extends Controller
{
	...
	protected function getForm()
	{
		...
		if (isset($this->request->post['product_tab'])) {
			...
			foreach ($data['product_tabs'] as $id => $tab) {
				if (isset($tab['description'][$language_id]['image']) && $tab['description'][$language_id]['image']) {
					$image = $this->model_tool_image->resize($tab['description'][$language_id]['image'], 100, 100);
				} else {
					$image = $this->model_tool_image->resize('no_image.png', 100, 100);
				}

				$data['product_tabs'][$id]['description'][$language_id]['image'] = $tab['description'][$language_id]['image'];
				$data['product_tabs'][$id]['description'][$language_id]['thumb'] = $image;
		}
